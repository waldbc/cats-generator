...
def join_layers(assets: str) -> list:
    """Loops through each layer folder, and chooses a layer from each folder based on the given raritiy
    weights. It then appends all the paths to a list which act as the final layers for the image. 
    """
    final_layers = []
...
def create_metadata(description: str, token_name: str, edition: int, final_layers: list):
    """Takes in some user data, along with the layers of the image and creates a metadata json for the 
    image. The json object can be used to provide token data to IPFS or third-party websites such as OpenSea.
    """
...
def main():
    ...
    token_name = input('Enter the name for your tokens. This will appear as the name for each image\n')
    ...
    for _ in range(amount):
        ...
        final_layers = join_layers(ASSETS)
